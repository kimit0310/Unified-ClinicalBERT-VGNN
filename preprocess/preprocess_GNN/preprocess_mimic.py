...
def build_seqex(enc_dict,
    ...
    for _, enc in enc_dict.items():
        ...
        seqex.context.feature['HADM_ID'].int64_list.value.append(
            int(enc.encounter_id))
...
def parser_fn(serialized_example):
    context_features_config = {
        ...
        'HADM_ID': tf.io.FixedLenFeature([1], tf.int64)
    }
    ...
    return batch_sequence, labels, batch_context['HADM_ID']
...
def tf2csr(output_path, partition, maps):
    ...
    for data in dataset:
        ...
        np_hadm_id.append(data[2].numpy()[0][0])
        sys.stdout.write('%d\r' % count)
        sys.stdout.flush()
        if len(np_hadm_id) % 1000 == 0:
            print(len(np_hadm_id))
    pickle.dump((csr_matrix(np.array(np_data)), np.array(np_label), np.array(np_hadm_id)),
