...
class GraphLayer(nn.Module):
    ...
    def __init__(self, in_features, hidden_features, out_features, num_of_nodes,
        ...
        self.a = clone_params(nn.Parameter(torch.rand(
            size=(1, 2 * hidden_features)), requires_grad=True), num_of_heads)
    ...
    def attention(self, linear, a, N, data, edge):
        ...
        )) / np.sqrt(self.hidden_features * self.num_of_heads))
    ...
    def forward(self, edge, data=None):
        N = self.num_of_nodes
        if self.concat:
            h_prime = torch.cat([self.attention(l, a, N, data, edge)
                                for l, a in zip(self.W, self.a)], dim=1)
        else:
...
class VariationalGNN(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_of_nodes, n_heads, n_layers,
        ...
        self.embed = nn.Embedding(
            self.num_of_nodes, in_features, padding_idx=0)
        self.in_att = clones(
    ...
    def data_to_edges(self, data):
        ...
        nonzero = torch.cat(
            (nonzero, torch.LongTensor([[length + 1]]).to(device)), dim=1)
        lengths = nonzero.size()[1]
    ...
    def forward(self, data):
        ...
        if self.none_graph_features == 0:
            outputs = [self.encoder_decoder(data[i, :])
