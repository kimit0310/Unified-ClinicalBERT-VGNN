...
train_data = pd.read_csv('train_matched.csv')
val_data = pd.read_csv('val_matched.csv')

positive_samples = train_data[train_data['LABEL'] == 1]
train_data = pd.concat([train_data, positive_samples], ignore_index=True)
...
class_weights = torch.tensor([0.18528, 0.81472]).to(device)
...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        labels = inputs.pop("labels").to(inputs[list(inputs.keys())[0]].dtype)  # Add .to() for data type conversion
